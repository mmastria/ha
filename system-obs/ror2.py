...
class RoR:
    ...
    def _can_unpark(self):
        return _is_parked() and self._is_mount_parked
    ...
    def status(self):
        ...
        self.log.debug('-- parked: %s (p:%s) - open: %s - closed: %s (s:%s) - mount_parked: %s (m:%s)' % (
            _is_parked(), p, self._is_open, self._is_closed, s, self._is_mount_parked, m))
        return '%s %s 0' % (p, s)
...
def park():
    ror = RoR()
    t = threading.Thread(target=ror.park)
    t.start()
    return
...
def open():
    ror = RoR()
    t = threading.Thread(target=ror.open)
    t.start()
    return
...
def close():
    ror = RoR()
    t = threading.Thread(target=ror.close)
    t.start()
    return
