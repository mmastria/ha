...
bool ArmPlat::setMovePower(uint16_t nmovepow)
{
    if ( port == -1 ) {
        LOGF_DEBUG("setMovePower - port error %d", port );
        return false;
    }

    int rc;
    char cmd[SLP_SEND_BUF_SIZE]={0};
    sprintf(cmd, "!step movepow %d %d#", port, nmovepow );

    if ( slpSendRxInt( cmd, &rc ) )
    {
        if ( rc == 0 ) {
                LOGF_DEBUG("setMovePower - send ok %d", rc );
                return true;
	}
    }
    LOGF_DEBUG("setMovePower - send error %d", -1 );

    return false;
}

bool ArmPlat::setStopPower(uint16_t nstoppow)
{
    if ( port == -1 ) {
        LOGF_DEBUG("setStopPower - port error %d", port );
        return false;
    }

    int rc;
    char cmd[SLP_SEND_BUF_SIZE]={0};
    sprintf(cmd, "!step stoppow %d %d#", port, nstoppow );

    if ( slpSendRxInt( cmd, &rc ) )
    {
        if ( rc == 0 ) {
                LOGF_DEBUG("setStopPower - send ok %d", rc );
                return true;
	}
    }
    LOGF_DEBUG("setStopPower - send error %d", -1 );
...
bool ArmPlat::setPort( uint16_t newport )
{
	...
	setMovePower( (uint16_t)MOVEPOW );
	setStopPower( (uint16_t)STOPPOW );
...
IPState ArmPlat::MoveAbsFocuser(uint32_t targetTicks)
{
    ...
    setMovePower( (uint16_t)MOVEPOW );
    LOGF_DEBUG("step movepow setted %d", MOVEPOW );
    setStopPower( (uint16_t)STOPPOW );
    LOGF_DEBUG("step stoppow setted %d", STOPPOW );
